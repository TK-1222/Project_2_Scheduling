...
class FFSASchedulingEnv(gym.Env):
    ...
    def __init__(self, config: InstanceConfig):
        ...
        self._assemblable_products_cache = None
    ...
    def _assemblable_products(self) -> List[int]:
        """조립 가능 제품 목록: pool 완비 + inactive final_job 존재 + idle 조립 기계 존재 (캐싱 적용)"""
        if self._assemblable_products_cache is not None:
            return self._assemblable_products_cache
        ...
        self._assemblable_products_cache = result
        return result
    ...
    def _estimate_comp_completion(self, comp_job_id: int) -> float:
        ...
        if comp_job_id in self._pool_blocked:
            ...
            return max(self.current_time, last_op.completion_time if last_op.completion_time is not None else self.current_time)
